...
export function OnboardingBeta() {
  ...
  const handleSubmitTrack = async (e: React.FormEvent) => {
    ...
    await createTrack({
      name,
      bg: trackColor.bg,
      border: trackColor.border,
      fg: trackColor.fg,
      darkBg: trackColor.darkBg,
      darkFg: trackColor.darkFg,
    });
    setSubmitting(false);
    setStep("column");
  };
  ...
  const addPendingColumn = () => {
    ...
    if (!name) return;
    if (pendingColumns.includes(name)) {
  ...
  const handleConfirm = async () => {
    if (pendingColumns.length === 0) {
      setError("Adicione ao menos uma coluna antes de continuar.");
      return;
    }
    setSubmitting(true);
    setError(null);
      ...
      <div className="w-full max-w-md">
        ...
        {step === "track" && (
          ...
          >
            ...
            <p className="text-xs text-muted-foreground">
              Trilhas são as áreas em que você organiza seu trabalho (ex.: Trabalho, Estudos,
              Projetos pessoais). Você pode criar mais depois.
            </p>
            <input
              autoFocus
              value={trackName}
                ...
                {TRACK_COLOR_PRESETS.map((c) => (
                  <button
                    ...
                    style={{
                      ...
                      outline:
                        c.bg === trackColor.bg ? `2px solid ${c.fg}` : "2px solid transparent",
                      outlineOffset: "2px",
                    }}
                  />
                ))}
        ...
        {step === "column" && (
          <div className="flex flex-col gap-4 rounded-lg border bg-card p-6">
            ...
            <p className="text-xs text-muted-foreground">
              Colunas são os estágios pelos quais os cards passam (ex.: A fazer, Em andamento,
              Concluído). Adicione quantas quiser.
            </p>
            <div className="rounded-sm border border-dashed bg-muted/40 px-3 py-2 text-[11px] text-muted-foreground">
              As colunas criadas aqui serão o padrão do sistema e podem ser editadas a qualquer
              momento.
            </div>
              ...
              <ul className="flex flex-col gap-1">
                {pendingColumns.map((name) => (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={() => removePendingColumn(name)}
...
function Indicator({ active, done, label }: { active: boolean; done: boolean; label: string }) {
  const base =
    "flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] uppercase tracking-widest";
  const cls = done
